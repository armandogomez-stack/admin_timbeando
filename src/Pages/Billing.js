...
function Index(props) {
	...
	useEffect(() => {
		...
		const getBillingItems = async () => {
			setLoading(true)
			try {
				const { data } = await axios.get(base_url(api_uri, `api/billing/list`))
				setItems(data)
						...
						<div className="card-header border-bottom">
							<div className="row">
								<div className="col-lg-4 ">
									<h4 className="card-title mt-3 float-left">Facturacion</h4>
								</div>
									...
									<table className="table mg-b-0 text-md-nowrap text-nowrap">
										<thead>
											<tr className="search-tr" >
												<th className="wd-5p serial-num text-center">#</th>
												<th>Concepto</th>
												<th>Estado</th>
												<th>Monto</th>
												<th>Fecha</th>
										...
										<tbody id="contact-table">
											...
											{items.map((item, key) => {
												...
												<td className="serial-num text-center">1</td>
